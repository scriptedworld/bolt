...
pub struct Task {
    ...
    /// The retired `jig` field, kept only to refuse it well, by FR-5.22.
    ///
    /// FR-5.18 makes composition a command line: a task running another jig
    /// writes `bolt` in its command like any other tool. Reading the field is
    /// what lets the refusal name it and say what replaced it, which serde's
    /// `missing field command` could not.
    #[serde(default)]
