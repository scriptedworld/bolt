...
| FR-1.14 | File handling is two calls, `load_formatted_file` and `save_formatted_file`, each taking the path, the schema to hold the file to, and the function that does the loading or saving. Validation sits in the signature: there is no way to read or write without naming what the file must conform to, which makes FR-1.9 a property of the library's surface rather than a discipline every call site has to keep. | [A] |
| FR-1.15 | That function defaults to the safe YAML pair and is substituted in a test, so the boundary is declared rather than reached for and the validation paths can be exercised against something other than a real file. | [A] |
| FR-1.15a | Which function is passed is also what selects the format. There is no separate format argument, and a caller wanting something other than YAML passes something other than the YAML pair. | [A/D] |
