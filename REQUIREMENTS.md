...
| FR-5.14b | It overrides the base and nothing else. `{project_root}` was already the root, `config-dir` still resolves as it did, and the child's output still sits in its task's work directory. A jig needing to stand at the root is one narrow claim and it buys one narrow exception. | [D] |
