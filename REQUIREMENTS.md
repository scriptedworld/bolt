...
| FR-4.4a | That is a rule about every task, because every task is a command task by FR-5.18. A task composing bolt over a subdirectory that is not there matches nothing and fails by FR-4.4b, and says `optional` when a missing subproject is expected. There is no second kind of task with a second rule to remember. | [A/D] |
