...
| FR-3.14 | A jig's task set is fixed by the jig. No task is conditional on anything read at run time, so two runs of one jig show the same tasks and differ only where the tree differed. State cannot be relied on to be the same between runs, and a task set that varied with it would make two results incomparable without either of them saying so. | [A] |
| FR-3.14a | A task wanted in some directories and not others is a separate jig, listed by the jigs that want it and left out of the ones that do not. FR-5.12 already has one jig named by many jig tasks, so selecting per directory is what a project jig is for, and the selection is readable in the jig rather than decided during the run. | [A] |
| FR-3.14b | FR-4.4's empty selection is not an exception to this. What a task matched is a property of the tree being run over, which is the run's input and is recorded in its manifest. A condition read at run time is neither. | [A/D] |
| FR-3.15 | A jig may declare `definitions`, giving a default to any placeholder it uses. The block is optional, as is any entry in it: a jig with no placeholders writes none, and one deliberately leaving a value to its adopter names the placeholder in a command and defines nothing. A shared jig that does define its defaults ships working values instead of requiring every adopter to supply a full set. | [D] |
...
| FR-4.16a | `--definitions <name>` names one, read from the config directory as `bolt.<name>.definitions.yaml`. A jig is `bolt.<name>.yaml` in the same place by FR-3.9, so a definitions file is adopted, linked and spoken of exactly as a jig is, and `link-jigs` can distribute a shared one. | [D] |
...
| FR-4.16c | A definitions file is one level of names, and a value is a scalar. The file's name carries the scope nesting would have expressed, and a list has no defined spelling on a command line, a question `{all_paths}` answered for paths and nothing else needs yet. | [D] |
| FR-4.17 | The merge is a successive replacement by key: each layer sets the keys it names and leaves every other key standing. Nothing is deep-merged, appended to or combined, so a project overriding one detail writes that one line and inherits everything else the jig shipped. | [D] |
...
| FR-4.17b | A value written as a relative path resolves under FR-2.9, against the base of the run that read the file rather than against wherever the file sits. `requirements: ../REQUIREMENTS.md` therefore reaches the repository root from a run based at `go/`, and one contract serves both packs from one file. | [D] |
...
| FR-4.18a | It is checked when `requires` is, under FR-3.10b, so a jig run where nothing defines what it needs refuses in the first second rather than partway through a gate. | [D] |
...
| FR-5.13j | `definitions` names the child's definitions file, as `--definitions` does for an invocation from the command line. Left out, it inherits, so a child runs with the values its parent was given until a field says otherwise. | [D] |
...
| FR-5.17 | Inheritance is what keeps one adjustment in one place. Six Python subprojects are six jig tasks naming no definitions file, so all six run against the one the project named, and a seventh that genuinely differs names its own. Copying the same override into six places is the thing this exists to prevent. | [D] |
...
| FR-6.2e | It is still expected to leave the envelope where the default would, because FR-6.2b's name never varies and no flag says where it goes. An invocation that writes elsewhere has written nothing, and is the same condition as an adapter that wrote nothing. | [D] |
...
| FR-9.5g | The manifest records the whole mapping FR-4.16 built, and which of the three layers each key came from. FR-9.5c already puts every value bolt exposed there; a run whose jig carries overrides also needs which file won, because the same key means different things depending on that and the command line alone does not say. | [D] |
