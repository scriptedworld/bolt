...
| FR-2.3 | The directory is the run's base, and every input path a run acts on comes from inside it. Containment is how the input is formed, not a check applied to it afterwards. It is a claim about the walked input and about nothing else: the config directory is named separately by FR-2.8 and is routinely outside the base, `link-jigs` places shared jigs that resolve into another repository, `{project_root}` reaches above the base by FR-4.1, and the output directory may be anywhere by FR-2.6. A row claiming everything a run touches is inside the base would make four settled rows violations of it. | [A/D] |
