...
| FR-3.10e | It is that jig's inventory and not its children's. FR-5.1b keeps a parent from reading a child's content, so a jig composing another says `bolt` in its `requires` and stops, and what the child needs is the child's own list checked when the child runs. A parent that gathered them up would be reading inside a jig it is only supposed to invoke. | [D] |
...
| FR-4.4a | That is a rule about every task, because every task is a command task by FR-5.18. A task composing bolt over a subdirectory that is not there matches nothing and fails by FR-4.4b, and says `allow-empty` when a missing subproject is expected. There is no second kind of task with a second rule to remember. | [A/D] |
...
| FR-10.8 | `--result-to-exitcode` makes bolt exit with what its envelope says rather than with whether it carried the run out. **Off unless named**, so FR-10.1's answer is what every caller that does not ask still gets, and nothing already written changes meaning. | [D] |
...
| FR-5.15 | 2026-08-28 | FR-4.4 and FR-4.4b. A composing task with no selection is a command task with no selection, and `allow-empty` is how one says it may run anyway. |
| FR-5.15a | 2026-08-28 | FR-4.4b by the same route. A subdirectory that is not there matches nothing, and FR-4.4c's `allow-empty` is the declaration that this is expected. |
...
| FR-4.4g | 2026-08-28 | FR-4.4c. A composing task has a selection like any command task, so `allow-empty` means what it means everywhere and is not an error. |
