...
| FR-4.17 | The merge is by key and the file wins. A key the file does not set keeps the jig's default, so a project overriding one detail writes that one line and inherits everything else the jig shipped. | [D] |
