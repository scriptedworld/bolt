...
12. Does a missing-directory refusal take the same shape as a depth refusal,
    ...
    non-zero?
13. What does a run report when the directory holds nothing any task matches?
    Every task is skipped and the run reports `success: true` having checked
    nothing.

## Adapters

14. The adapter writes `output.yaml`, so bolt did not write it and FR-1.8's
...
15. What happens when a declared evidence file was not produced? A tool that
...
16. Which adapter runs when a task names none?
17. How is an adapter resolved by name: a search path, a directory inside
    toolbox, a symlink placed by `link-jigs`?
18. What happens when an adapter exits non-zero, writes nothing, or writes
    something that will not parse? All three are FR-7.6's no-authoritative-
    result and may deserve different reasons.

## The envelope

19. Is `message` a required key on a reason object, or merely conventional?
    Something guaranteed makes every reason renderable by one consumer while
    leaving the rest of the object open.
20. In the merged mapping, is `args` the argv as executed, or the jig-declared
    arguments before substitution?
21. Does an envelope name its own task, or does the merge take the name from
    the work directory?

## The output directory

22. For a per-path task, does each execution's manifest record the whole matched
...
23. Does a task that executes exactly once still carry an ordinal?
24. Is a run's own base recorded anywhere a reader will find it? FR-9.5c puts
...
25. What does bolt do when `--output-dir` already holds a previous run?
26. Is `--output-dir` created if absent? A graph node's `.ephemera/` may not
    exist yet.
27. Where is `.bolt-<iso8601>` rooted when no output directory is named? Bolt
    reads no git, which argues for the working directory.
28. What spelling of ISO 8601? The strict form carries colons, which are legal
    here and hostile to a Windows checkout. Local time with an offset, or UTC?

## Failure

29. FR-4.10 has a missing binary carried by the reason rather than by a status.
...
30. An image is built from a jig, so a shared jig and its image cannot drift.
    ...
    that case?
31. If bolt fails partway, does it write a partial `result.yaml` or none?
    FR-7.5 argues for none, but a caller then cannot tell a crashed run from
    one that never started.
32. What exit statuses does bolt use, and what does each mean? Candidates: a
...
33. Is a task that could not execute recorded distinguishably from one that
    executed and failed?

## Nested jigs

34. Must a jig task's subdirectory exist? A written path in a shared jig can
...
35. A jig task has no command to read a mode off, so it runs once against its
    base. Worth confirming there is no case wanting a jig run per path.
36. Subprocess or in-process? Subprocess gives the bookkeeping files for free,
...
37. Is `statistics.source` a literal key, or a placeholder for the source's
    name used as the key? And what makes it a list rather than one object?
38. Do a task's own `evidence` paths survive into `result.yaml`, or does a
    reader follow the mapping's result filepath to the task's own envelope?
39. May `metadata` carry adapter-specific keys beyond `statistics` and
    `evidence`?
40. Is there a schema version field, given `success` is the only guarantee?
    What does a consumer do with a version it does not recognise?
41. FR-1.5 validates everything read as data, and the consequence of failing
...
42. Does `result.yaml` keep the envelope shape exactly, or carry keys a task
    envelope never has?

## Tasks

43. Are the jig errors FR-3.4b and FR-4.2 name caught when the jig is read, or
...
44. Is the environment handed to a task command inherited wholesale, filtered
    to an allowlist, or declared per task? Whatever the rule, the depth
    variable has to survive it.
45. Can a task be disabled in a jig without deleting it, and does a disabled
    task appear in the result?
46. FR-1.5 validates a jig. Does an unrecognised key fail it or warn? Failing
    makes a jig written for a newer bolt unusable by an older one; warning lets
    a typo pass as an ignored field.

## Adapters

47. With an explicit adapter invocation, are the same substitutions available,
    and is the envelope still expected in the same place?
48. Is a manifest written before the command runs or after it finishes? Before
...
49. May an adapter read the repository tree, or only the files it was handed?
...
50. Where is each limit declared? A task's belongs on the task; a run's could
    sit on the jig, on the command line, or both, and if both then which wins.
51. How is a timed-out child terminated, which signal and with what grace, and
...
52. What records the executions a task never reached? A per-path task cut off
...
53. Does a run that times out fold in the constituents that completed, or does
...
54. Is a task skipped for an empty selection recorded in `result.yaml`? Absence
    currently reads the same as never declared, so a green result can mean that
    nothing was checked.
55. Does a manifest record what the walk found, or only what the task matched?
...
58. Does a run over a large tree exceed `ARG_MAX` when `{all_paths}` expands?
...
59. §67's pre-commit overlay wants a gate over what changed, and a
...
60. Is there a shorthand for naming one jig at many bases? Nine Go subprojects
...
61. Does FR-5.12's whole-jig override survive separate location variables? A
    ...
    root.
62. Is a cycle detected by name, or only by depth exhaustion? A jig stack in
    the environment would name the cycle; a counter can only report a number.
63. Can a jig be referenced by version, or is it always whatever is on disk?

## Bounds and guards

64. Is the ancestry cross-check worth building? It closes `unset BOLT_DEPTH`,
...
65. Is the per-user cap on live runs wanted? If so, what is the default, and is
    the default off?
66. When parallel execution arrives, is the bound a per-run budget that
    descends with nesting rather than a shared counter?

## Composition and overlay

67. When a jig invokes another, what does the child inherit: environment,
    timeouts, the required default?
68. Can a parent override a field of a child's task, and at what granularity:
    the whole task, or one field?
69. Can a parent disable a task a shared jig declares, and is the omission
    visible in the result?
70. Is there a user-level or machine-level layer above the repository's jig?
    §67 describes exactly that for pre-commit, a repository policy plus an
    independent personal one.
71. What is the precedence when the same key is set at more than one layer, and
    are collections merged or replaced?

## Boundaries with the rest of the ecosystem

72. Does the per-file coverage policy of §20 live in an adapter, in a toolbox
    analyzer an adapter then reads, or in bolt? Which repository owns it
    decides whether it is a bolt requirement at all.
73. Is `result.yaml` what a ratchet node depends on directly, or does a node
    wrap it in something of its own?
74. Does bolt stamp the tree state §65 wants, or does that move to the caller?
    Bolt reads no git for anything else.
75. What holds the per-language implementations level? Validation itself is
